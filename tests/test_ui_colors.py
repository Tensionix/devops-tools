...
def test_devops_theme_catalog_contract() -> None:
    result = validate_theme_catalog(_data())
    assert result.ok, "\n".join(result.errors)


def test_devops_theme_core_order_and_ru_labels() -> None:
    themes = _themes()
...
def test_devops_theme_contract_allows_project_extensions_after_core() -> None:
    data = _data()
    themes = dict(_themes())
