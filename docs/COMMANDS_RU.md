...
- [DNS-доктор > Снимок состояния](#dns-доктор--снимок-состояния)
- [DNS-доктор > Починить имена и значок](#dns-доктор--починить-имена-и-значок)
- [DNS-доктор > Починить значок сети](#dns-доктор--починить-значок-сети)
- [DNS-доктор > Резолвер: от роутера (DHCP)](#dns-доктор--резолвер-от-роутера-dhcp)
- [DNS-доктор > Резолвер: сам роутер](#dns-доктор--резолвер-сам-роутер)
- [DNS-доктор > Резолвер: Cloudflare](#dns-доктор--резолвер-cloudflare)
- [DNS-доктор > Резолвер: Google](#dns-доктор--резолвер-google)
- [DNS-доктор > Резолвер: Quad9](#dns-доктор--резолвер-quad9)
- [DNS-доктор > Резолвер: Яндекс](#dns-доктор--резолвер-яндекс)
- [DNS-доктор > Резолвер: AdGuard](#dns-доктор--резолвер-adguard)
...
- [Hardware > Съёмные носители > Обнулить съёмный и сделать один раздел](#hardware--съёмные-носители--обнулить-съёмный-и-сделать-один-раздел)
- [Hardware > Съёмные носители > Открыть папку съёмных носителей](#hardware--съёмные-носители--открыть-папку-съёмных-носителей)
- [Сеть > Wi-Fi: статус / подключение > Раздача Wi-Fi](#сеть--wi-fi-статус--подключение--раздача-wi-fi)
- [Hardware > Автозапуск > Что запускается само](#hardware--автозапуск--что-запускается-само)
- [Hardware > Автозапуск > Открыть папку автозапуска](#hardware--автозапуск--открыть-папку-автозапуска)
...
### DNS-доктор > Снимок состояния

- Operation id: `dns_status`
- Описание: Только чтение: DNS на адаптерах, что предлагает роутер по DHCP, кто держит 53-й порт, кто в итоге спрашивает наружу и его обратное имя, разрешается ли имя и в каком состоянии служба значка сети.
- Риск: kind=`safe`, risk_level=`readonly`
- Параметры: нет.

### DNS-доктор > Починить имена и значок

- Operation id: `dns_repair`
- Описание: Поднимает лежачий локальный резолвер (ControlD, YogaDNS, AdGuard, dnscrypt); если его нет вовсе - возвращает адаптерам адреса роутера. Затем приводит службу значка сети к тому виду, в каком её держит сама Windows.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Починить значок сети

- Operation id: `dns_fix_icon`
- Описание: Только NlaSvc: включает активную пробу выхода в интернет, если её погасили, ставит службе автозапуск, поднимает её и перезапускает, когда оценка сети устарела.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Резолвер: от роутера (DHCP)

- Operation id: `dns_use_auto`
- Описание: Возвращает всем физическим адаптерам автоматический DNS - как задумано в той сети, куда включена машина.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Резолвер: сам роутер

- Operation id: `dns_use_router`
- Описание: Направляет адаптеры прямо на шлюз. Перед записью роутера спрашивают именем.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Резолвер: Cloudflare

- Operation id: `dns_use_cloudflare`
- Описание: Cloudflare 1.1.1.1 и 1.0.0.1 - быстрый, ничего не режет. Перед записью спрашивают именем.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Резолвер: Google

- Operation id: `dns_use_google`
- Описание: Google 8.8.8.8 и 8.8.4.4. Перед записью спрашивают именем.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Резолвер: Quad9

- Operation id: `dns_use_quad9`
- Описание: Quad9 9.9.9.9 и 149.112.112.112 - режет вредоносные имена. Перед записью спрашивают именем.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Резолвер: Яндекс

- Operation id: `dns_use_yandex`
- Описание: Яндекс 77.88.8.8 и 77.88.8.1 - ближе прочих, есть свои фильтры. Перед записью спрашивают именем.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.

### DNS-доктор > Резолвер: AdGuard

- Operation id: `dns_use_adguard`
- Описание: AdGuard 94.140.14.14 и 94.140.15.15 - режет рекламу и счётчики. Перед записью спрашивают именем.
- Риск: kind=`dangerous`, risk_level=`system_change`
- Параметры: нет.
...
### Hardware > Съёмные носители > Обнулить съёмный и сделать один раздел

- Operation id: `storage_removable_prepare`
- Описание: Открыть инструмент во внешней консоли: покажет диски, спросит номер и объём выбранного как подтверждение, затем обнулит и отформатирует.
- Риск: kind=`dangerous`, risk_level=`destructive`
- Параметры: нет.

### Hardware > Съёмные носители > Открыть папку съёмных носителей

- Operation id: `storage_removable_open_folder`
- Описание: Открыть папку disk-removable с README и сценариями; без изменений дисков.
- Риск: kind=`safe`, risk_level=`readonly`
- Параметры: нет.

### Сеть > Wi-Fi: статус / подключение > Раздача Wi-Fi

- Operation id: `network_wifi_hotspot`
- Описание: Показать, включить или выключить раздачу Wi-Fi — тот же переключатель, что в параметрах Windows. Имя сети и пароль остаются в параметрах Windows, здесь только подъём. При загрузке машины не поднимается: интерфейсу раздачи нужен вошедший пользователь.
- Риск: kind=`dangerous`, risk_level=`modifies`
- Параметры:
  - `hotspot_action` — **Что сделать** (type=`select`, default=`status`). Варианты: `status` — Показать состояние; `on` — Включить раздачу; `off` — Выключить раздачу

### Hardware > Автозапуск > Что запускается само

- Operation id: `startup_kit_open`
- Описание: Открыть инструмент во внешней консоли: задачи планировщика со входом и загрузкой, обе папки автозагрузки, Run и RunOnce — одним списком, с задержками. Заводит записи, находит, что уже запускает ту же программу, и убирает пересечения.
- Риск: kind=`dangerous`, risk_level=`modifies`
- Параметры: нет.

### Hardware > Автозапуск > Открыть папку автозапуска

- Operation id: `startup_kit_open_folder`
- Описание: Открыть папку startup_kit с README и сценариями; ничего не меняет.
- Риск: kind=`safe`, risk_level=`readonly`
- Параметры: нет.
