...
## Project Cleaner Трогает Не То

Проблема: root-cleaner нужен перед архивом или переносом, но можно ошибиться в границе: оставить скачанный runtime/wheelhouse/fzf/install-cache внутри источников или, наоборот, удалить скрипты, конфиги, документацию и соседние bundle-папки.

Решение: `cleanup_project.cmd` должен сохранять scripts/config/docs/tracked license docs/folder structure, но удалять всё generated/downloaded: `runtime`, `wheelhouse`, `release`, `install\download`, `system_core\powershell`, `system_core\fzf.exe`, logs, reports, input/output/workspace/data/backup contents и Python caches. Соседние инструменты bundle не трогать. `cleanup_project.cmd /BACKUP` остаётся отдельным узким режимом, когда нужно очистить только backup, с собственным `Y/N/Q`.
...
## Hosts Depatch Через Редактирование Строк

Проблема: кажется логичным выключить hosts override удалением или комментированием строки. Но `hosts` - системный файл, и построчная пересборка может поменять encoding, line endings, trailing blank lines, timestamp и чужие комментарии.

Решение: для `Hosts and Bitrix` depatch должен быть побитовым. `Enable override` сохраняет `hosts_prepatch_....bak` и пишет имя backup в managed-комментарий `AUDION_BITRIX_HOSTS`. `Disable override` читает `backup=...` и копирует этот backup поверх системного `hosts`. Если metadata или файл отсутствуют, лучше отказаться от "почти отката" и использовать аварийный `Restore original hosts`.

## Публичный IP В Detect Для Hosts

Проблема: DNS lookup может вернуть публичный IP. Если автоматически подставить его в `hosts`, можно случайно закрепить внешний маршрут вместо локального on-prem Bitrix.

Решение: `Detect current endpoint` должен заполнять `IP address` только для локальных/private адресов: `10/8`, `172.16/12`, `192.168/16`, loopback, link-local и локальные IPv6 ranges. Публичные IP показываются в журнале, но не меняют GUI-поля.
...
## Dynamic Select Оставил Пустой Экран

Проблема: `options_source` вернул новый список, а старое значение поля больше в него не входит. NiceGUI/Quasar может не отрисовать нормальный select, и пользователь видит только заголовок `Параметры`, без действий ниже.

Решение: перед рендером нормализуйте значение `select`/`radio` к одному из текущих `options`, обычно к первому варианту. Ошибку provider показывайте как field-level message или error option, а не роняйте весь command pane.

## Путь Оставлен Как Голое Text-Поле

Проблема: GUI заставляет пользователя писать `E:\...` руками или делать `cd e:\` в терминале, хотя это как раз то, что GUI должен упростить.

Решение: для новых форм используйте `type: "folder"` и `type: "file"`. Для выбранного файла в терминальной строке удобно ставить cwd в папку файла и подставлять полный путь файла в команду.
...
## Ручная Правка Защищённых Defaults Windows

Проблема: хочется "починить дефолты" прямой правкой `HKCU...\UserChoice`, удалением hash или отключением защитного слоя Windows. На Windows 11 это быстро превращается в гонку с UCPD, сбросами defaults и непредсказуемыми уведомлениями.

Решение: для Default Apps Guard использовать официальный admin path: DISM export/rescan/import `AppAssociations.xml`, HKLM policy `DefaultAssociationsConfiguration`, `gpupdate`, затем sign out/sign in или reboot. Личный выбор пользователя (`UserChoice`) проект только читает — для снимка и сравнения; ни своей записи хеша, ни сторонней утилиты для этого в проекте нет. UCPD в проекте диагностируется, но не обходится молча.

Отдельная запись про то, почему выброшен SetUserFTA, — в `MEMORY.md`.

## Терминал Пустой, Хотя Операция Отработала

Проблема: строки терминала отправлялись как `ui.run_javascript("requestAnimationFrame(() => window.audionTerminalReset(...))")`. Сообщения доходили до браузера по сокету, статус и прогресс обновлялись, а `pre.audion-terminal` оставался пустым. Причина: когда окно не композитит кадры (свёрнуто, полностью перекрыто, скрытая панель предпросмотра), `requestAnimationFrame` не вызывается вообще, и колбэк с текстом просто не выполняется.

Решение: содержимое терминала — это состояние элемента, а не побочный эффект JavaScript. Пишем `log_view.content = terminal_lines_html(state["lines"])` через Python API NiceGUI (см. `nicegui/llms.md`: обновлять содержимое из JS запрещено, модель Python расходится с DOM). Полная перерисовка ограничена `TERMINAL_MAX_LINES`, поэтому дешёвая, и она же чинит восстановление терминала после переподключения сокета. JavaScript остаётся только для прокрутки: `MutationObserver` держит низ, если пользователь не прокрутил вверх и не выделяет текст.
