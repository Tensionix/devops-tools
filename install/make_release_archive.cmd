...
title Audion DevOps Tools - Make Release Archive
...
robocopy "%ROOT%" "%STAGE_PROJECT%" /E /R:1 /W:1 /NFL /NDL /NJH /NJS /NP /XD "%ROOT%\.git" "%ROOT%\backup" "%ROOT%\release" "%ROOT%\.venv" "%ROOT%\.venv_latest" "%ROOT%\venv" "%ROOT%\._runtime" "%ROOT%\__pycache__" "%ROOT%\.pytest_cache" "%ROOT%\.mypy_cache" "%ROOT%\.ruff_cache" /XF "%ROOT%\api_key.txt" "%ROOT%\api_key_openai.txt" >nul
set "RC=%errorlevel%"
if %RC% GEQ 8 goto ERR_STAGE

call :PRUNE_DEV_ARTIFACTS
if exist "%STAGE_PROJECT%\install\init_folders.cmd" call "%STAGE_PROJECT%\install\init_folders.cmd" >nul 2>nul
...
echo   - Verify project backup snapshots are not included; release contains empty backup structure only
